...
// Cache for processed CSS to avoid recomputation
let cachedStyleElement: HTMLStyleElement | null = null

/**
 * Generates a style element with adjusted CSS to work correctly within a Shadow DOM.
 * Uses memoization to avoid reprocessing CSS on every call.
 *
...
export const getStyle = (): HTMLStyleElement => {
  ...
  if (cachedStyleElement) {
    return cachedStyleElement.cloneNode(true) as HTMLStyleElement
  }
  ...
  // Cache the processed style element
  cachedStyleElement = styleElement.cloneNode(true) as HTMLStyleElement
...
let cachedSvgElement: SVGElement | null = null

function createCachedSvgIcon(): SVGElement {
  if (cachedSvgElement) {
...
function createNFTButton(): HTMLButtonElement {
  ...
  btn.addEventListener("click", (e: MouseEvent) => {
    ...
    log.debug("Dispatching event:", {
      eventType: appconfig.TOGGLE_EVENT_TYPE,
      detail: event.detail
    })
  ...
  try {
    ...
    const verificationTimeout = setTimeout(() => {
      if (fileInput.files && fileInput.files.length > 0) {
        log.debug(
          "File upload verification: Files still attached - checking Twitter processing"
        )
        ...
        if (hasUploadUI) {
          ...
          log.debug(
            "⚠️ File upload UNCLEAR: Files attached but no UI changes detected"
          )
        }
      } else {
        log.debug(
          "✅ File upload SUCCESS: Files cleared, likely processed by Twitter"
        )
      }
    }, 150)
...
function handleDragOver(event: DragEvent) {
  ...
  if (event.dataTransfer && event.dataTransfer.types.includes("Files")) {
    ...
    if (
      rect.width > 200 &&
      rect.height > 100 &&
      !target.querySelector(".nftory-drop-hint")
    ) {
...
async function handleDrop(event: DragEvent) {
  ...
  try {
    ...
    if (event.dataTransfer.files && event.dataTransfer.files.length > 0) {
      ...
      log.debug("Processing dropped NFT file:", {
        name: file.name,
        size: file.size,
        type: file.type
      })
      ...
      log.debug(
        "No valid drop data found. Available types:",
    ...
    try {
      ...
      elements.forEach((element) => {
        ...
        if (
          ...
          element.hasAttribute("data-nftory-dropzone")
        ) {
          return
        }
